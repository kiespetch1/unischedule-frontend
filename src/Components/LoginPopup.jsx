...
const LoginPopup = forwardRef((props, ref) => {
    const { updateAuthorization } = useContext(Context);
    ...
    const handleLogout = async () => {
        await fetch('https://localhost:7184/api/users/logout', deleteRequestOptions);
    ...
    const checkAuthorization = async () => {
        ...
        if (value === "true") {
            ...
            const response = await fetch('https://localhost:7184/api/users/get-email', getRequestOptions);
            if (response.ok) {
