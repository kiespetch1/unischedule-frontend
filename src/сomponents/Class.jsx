...
import {
    ...
    POST_REQUEST_OPTIONS_WITH_AUTH,
...
const Class = forwardRef(
    (
        ...
        const [isTeacherFilterActive, setIsTeacherFilterActive] =
            useState(false) //флаг для понимания, когда мы фильтруем учителей, а когда нет
        const [locationFilter, setLocationFilter] = useState('') //фильтр локаций, он же служит отображаемым именем
        const [isLocationFilterActive, setIsLocationFilterActive] =
            useState(false) //флаг для понимания, когда мы фильтруем локации, а когда нет
        ...
        const handleLocationSave = () => {
            if (newLocationInfo !== '' && newLocationType !== '') {
                ...
                fetch(
                    '/api/locations?' + typeQuery + locationQuery,
                    POST_REQUEST_OPTIONS_WITH_AUTH
                )
                    .then((response) => {
                        if (!response.ok) {
                ...
                fetch(
                    ...
                    if (response.ok) {
                        ...
                        if (response.status === 404) {
                            toast.error(
                                'Не удалось найти эту пару. Возможно она уже удалена?'
                            )
                        } else toast.error('Не удалось удалить пару.')
                    }
                })
            }
            setIsDeleted(true)
        ...
        if (isNew) {
            ...
            teacher = dayData.teachers.find(
                (teacher) => teacher.id === dayData.classes[order - 1].teacherId
            )
            locationTypeSc = dayData.locations.find(
                (location) => location.id === locationId
            ).locationType
            classroom = dayData.locations.find(
                (location) => location.id === locationId
            ).classroom
            link = dayData.locations.find(
                (location) => location.id === locationId
            ).link
        }
        ...
        useEffect(() => {
            //инициализация начальных значений для редактирования пары
        ...
        useEffect(() => {
            function generateParams(params) {
                return Object.keys(params)
                    ...
                    .map(
                        (key) =>
                            `${key}=${encodeURIComponent(params[key].value)}`
                    )
                    .join('&')
            }
                        ...
                        fetch(url, PUT_REQUEST_OPTIONS_WITH_AUTH)
                            .then((response) => {
                                if (
                                    ...
                                    console.log(
                                        'Request was successful, but no content returned.'
                                    )
                                    return response.json()
                                } else {
                                    toast.error(
                                        'Не удалось создать пару. Убедитесь что все поля заполнены корректно.'
                                    )
                                }
                            })
                            .then((data) => {
        ...
        useEffect(() => {
            //для закрытия окна по клику вне его зоны
        ...
        return (
            <div>
                <section
                    className={order === '1' ? 'day-block-top' : 'day-block'}
                ...
                >
                    <div className="main-content">
                        <div
                            className={
                                isActive ? 'info-row editing' : 'info-row'
                            }
                        >
                            {/*строка времени*/}
                            {isActive ? (
                            ...
                            {isActive ? (
                                ...
                                <div className="class-type">
                                    {parseInt(newClassType) === 0
                                        ? 'Лекция'
                                        : parseInt(newClassType) === 1
                                          ? 'Практика'
                                          : parseInt(newClassType) === 2
                                            ? 'Лаб. работа'
                                            : 'Ошибка'}
                                </div>
                            ) : (
                            ...
                            {isActive ? (
                                <div
                                    className={
                                        isWeekTypeEditing
                                            ? 'week-type-icon editing'
                                            : 'week-type-icon'
                                    }
                                    onClick={handleClassPropertiesChange}
                                >
                                    {newWeekType === 0 ? null : newWeekType ===
                                      1 ? (
                                        windowWidth <= 930 ? (
                                            <OddWeekIconSmall
                                                style={signSmallStyle}
                                            />
                                        ) : (
                                    ...
                                    {newSubgroup === 0 ? null : newSubgroup ===
                                      1 ? (
                                        windowWidth <= 930 ? (
                                            <FirstSGSmall
                                                style={lastSignSmallStyle}
                                            />
                                        ) : (
                                ...
                                <>
                                    ...
                                    {subgroup === 0 ? null : subgroup === 1 ? (
                                        ...
                                        windowWidth <= 930 ? (
                                            ...
                                            <SecondSG style={lastSignStyle} />
                                        ) //этому дерьму нужен рефакторинг
                                    ) : null}
                                </>
                            )}
                        </div>
                        ...
                        >
                            ...
                            {isActive ? (
                                <input
                                    className="class-name editing"
                                    id="class-name-input"
                                    value={newClassName}
                                    onChange={handleClassNameChange}
                                ></input>
                            ) : isEditing ? (
                                <div className="class-name">{newClassName}</div>
                            ) : (
                        ...
                        >
                            ...
                            {isActive ? (
                                <input
                                    className="class-teacher editing"
                        ...
                        <div>
                            {(locationTypeSc === 0 ||
                                ...
                                (containsLetters(classroom) ||
                                    containsLetters(newLocationInfo)) &&
                                parseInt(newLocationType) !== 1 &&
                                (isActive ? (
                                    <input
                                        className="irl-letters-location editing"
                                        id="classroom-input"
                            ...
                            {(locationTypeSc === 0 ||
                                ...
                                (isActive ? (
                                    <input
                                        className="irl-location editing"
                                        id="classroom-input"
                                        value={locationFilter}
                                        onChange={handleLocationFilterChange}
                                    ></input>
                            ...
                            {(locationTypeSc === 1 ||
                                ...
                                (isActive ? (
                                    <input
                                        className="distant-location editing"
                                        id="classroom-input"
                                        value={locationFilter}
                                        onChange={handleLocationFilterChange}
                                    ></input>
                ...
                {isActive ? (
                    <div className="class-edit-panel-container">
                        {isWeekTypeEditing && (
                            <ClassPropertiesPopup
                                ref={popupRef}
