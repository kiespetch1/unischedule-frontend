...
const NotificationPopup = forwardRef(({ groupName, groupId }, ref) => {
    ...
    const [groupIdDuplicate, setGroupIdDuplicate] = useState(
        groupId || defaultGroupId
    )
    const [groupNameDuplicate, setGroupNameDuplicate] = useState(
        groupName || ''
    )
    ...
    const loadNotifications = useCallback(
        (page) => {
            ...
            const fetchNotifications = () => {
                fetch(
                    `/api/notifications?groupId=${groupIdDuplicate}&page=${page}`,
                    GET_REQUEST_OPTIONS
                )
                    .then((response) => response.json())
                    .then((data) => {
            ...
            <div ref={ref} className="notification-container">
                ...
                >
                    <p className="notification-header-text">
                        Выберите группу чтобы увидеть уведомления
                    </p>
                    <button
        ...
        <div ref={ref} className="notification-container">
            <div className="notification-header">
                <div className="notification-inner-header">
                    ...
                    <p
                        className="notification-group-header-text"
                    ...
                    <button
                        onClick={enableGroupChooser}
                        className="notification-secondary-header-text"
                    >
                        сменить группу
                    </button>
                </div>
            ...
            <div>
                ...
                {!isLoading &&
                    groupedNotifications.length > 0 &&
                    groupedNotifications.length !== notificationsCount && (
                        <div
                            style={{
                                display: 'flex',
                                justifyContent: 'center',
                        ...
                        >
                            <button
                                ...
                                onClick={handleNotificationsLoad}
                            >
